...
### Addendum — Field-level hot-apply gating (2026-04-24, follow-up to PR-8b)

PR-8b deliberately returned `restart_required: true` unconditionally because the always-honest answer is "nothing hot-applies until restart" — `build_session()` captures settings into closures at startup, and the route didn't rebind `session.components.settings` in memory either. Both halves were punted.

This follow-up closes the loop:

- **Selective rebind on save.** `PUT /api/settings` now replaces `session.components.settings` with the validated new instance (preserving the runtime-injected `jarvis_dir`). The rebind is what makes "hot-apply" real — without it, even fields that are re-read per turn would keep seeing the old object.
- **`HOT_APPLY_PATHS` whitelist.** A curated frozenset (`packages.core.settings.HOT_APPLY_PATHS`) of dotted prefixes that are truly hot because the running session reads them off `components.settings` every turn: currently `summarization.*` (read at the top of each `_run_one_turn` in `apps/gui/server/bridge.py`) and `paths.prompt_history_dir` (read in every `/api/agents/*/prompt*` route call). Additions require grepping for the field in `apps/gui/server`, `apps/cli/main.py`, and `packages/core` to confirm it isn't captured into a closure at `build_session()` time.
