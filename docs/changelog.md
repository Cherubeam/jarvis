...
### Tests
- **GUI mutation-test sweep, third pass — pending CI confirmation.** Targeted four small-survivor concentrations called out in PR #26's "easiest next batch": `routes.home` formatters (`_greeting` / `_day_label` / `_task_to_dict` / `_flatten_tasks`), `routes.agent_includes` helpers (`_guard_placeholder` parametrized like `_guard_agent_id`, plus `_meta_dict` / `_repo_rel` / `_affects_agents` / `_editable_for` / `_history_key` / `_shared_dir_for`), `routes.agents` helpers (`_load_meta_dict` / `_get_write_lock` / `_history_root` and the `_meta_path` / `_agent_dir` / `_system_prompt_path` chain), and `packages/core/settings.py` schema helpers (`_inline_refs` cycle/missing-def/non-defs-ref branches, `_diff_dict` recursion + dynamic-key passthrough, `_diff_paths` dotted-leaf shapes, `dereferenced_schema` smoke). +91 unit tests across 4 files; full local subset 508 → 611 tests. Mutation-kill numbers will be appended once Linux CI workflow run completes. Untouched modules in scope: `bridge` / `prompt_history` / the four no-tests entry points — those are bigger lifts deferred to follow-up PRs.
