...
### Tests
- **GUI mutation-test sweep, eighth pass (history.index) — pending CI confirmation.** Targets `apps.gui.server.history.index` (98 survivors after pass 7 — biggest single concrete target now that bridge.py is under 200). Inspected each survivor via `mutmut show`; 5 `# pragma: no mutate` annotations on the `logger.debug`/`warning` lines (`couldn't mkdir`, 3× `skipping ...`, `get(...) read failed`, `delete(...) unlink failed`) excluded ~50 equivalent string-mutation mutants from generation. +16 unit tests covering: unicode JSON content (forces `encoding="utf-8"` lock-in for both `_parse_file` and `get`); `list()` no-kwarg defaults (date='all', sort='recent', limit=200, offset=0) + sort-key `or 0` / `or 0.0` defaults with zero/low-value entries; `_build_summary_dict` `data.get(default)` lock-in for messages/metrics/model and exact-10-char boundary tests for both session_start and stem date extraction + `dom and agents and ...` conjunction test; `delete()` returns False (not True) on OSError; `_refresh_sync` mid-loop OSError on stat() skips via continue (defeats `continue → break`) + mkdir(parents=True, exist_ok=True) lock-in across two refreshes; `_in_date_range` boundary tests; `facets` tool-counting branch (existing tests had no tool data — covers the `tool_counts.get(t, 0) + 1` accumulator and the `(-kv[1], kv[0])` sort key). Full subset 683 → 699 passing.
