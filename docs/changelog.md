...
### Tests
- **GUI mutation-test sweep, fifth pass — pending CI confirmation.** Closes the genuine test gaps surfaced by inspecting pass-4 survivors via `mutmut show` (after working around the macOS fork-segfault by running `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES uv run mutmut run --max-children 1` long enough to populate the local mutant cache). Initial "pragma sweep" hypothesis was wrong — most apparent equivalent-mutant clusters were actually under-asserted helpers. Per-helper plan: `agent_includes._lookup` (14) gets `exc.detail` exact-string assertions on every 404 path + new unknown-agent-on-GET coverage; `_row_for` (6) gets `path` / `last_modified_iso` shape assertions + missing-file null-coercion test; `_affects_agents` (2) gets meta_path=None and filename-miss skip-vs-break tests; `_meta_dict` / `_load_meta_dict` (~22) get unicode content (forces utf-8 encoding choice) plus `# pragma: no mutate` on the equivalent logger.debug lines; `settings._classify_error` (8) gets bare-`type:object` schema test + array-without-items test, plus pragmas on two `current = None` assignments where mutating to other non-dict constants is provably equivalent; `packages.core.settings` (8) gets multi-key-dynamic dict tests + cycle-through-nested-collection tests, plus pragma on `dereferenced_schema` `pop("$defs", {})` default. +13 new unit tests, 5 pragma annotations across 4 source files. Full local subset clean (208 passing across the touched files; one pre-existing date-window flake in `test_detail_cost_14d_sums_only_agent_sessions` is unrelated). Expected post-CI: survivors 510 → ~460, kill rate 83% → ~85%.
