...
### Tests
- **GUI mutation-test sweep, fourth pass — pending CI confirmation.** Targets `apps/gui/server/routes/settings.py` helpers — the easiest untouched concentration after PR #27 (41 survivors across `_classify_error` 21, `_normalize_validation_errors` 14, `_get_write_lock` 4, `_has_managed_header` 2). +31 unit tests covering: schema-walking dispatch (scalar leaf, nested field, object boundary, int-segment-via-items, additionalProperties, mid-walk None, empty loc, properties-without-type-object), pydantic ValidationError wrapping (loc/card_loc/kind/msg/type passthrough, list serialisation of tuples, defensive `.get()` for missing keys, multi-error classification), file-content sentinel detection (missing file, directory, blank file, blank-only, leading-blanks, mismatch, whitespace-stripped, partial match, OSError swallowed), async lock singleton (creation, cache-hit, preexisting, under-contention serialisation), and `_local_yaml_path` literal segments. Full local subset 611 → 642 tests, all green in ~3 s. Mutation kill-rate delta will be appended once Linux CI workflow run completes. Bridge work (`_run_delegation` 193 + `_run_one_turn` history-summarization branch ~40) split out into a separate follow-up PR — extending `test_bridge_run_turn.py` with delegation fixtures is a meaningful enough lift to warrant its own scope.
