...
_SAMPLE_SETTINGS = Settings(
    ...
    routing=RoutingSettings(enabled=True, simple_threshold=200, complex_threshold=800),
)
...
class TestClassifyQuery:
    ...
    def test_short_greeting_routes_to_fast(self):
        preset, reason, confidence = classify_query("Hello!", _SAMPLE_SETTINGS)
        assert preset == "fast"
        assert reason == "short simple query"
        assert confidence == 0.8

    def test_short_question_routes_to_fast(self):
        preset, reason, confidence = classify_query("What time is it?", _SAMPLE_SETTINGS)
        assert preset == "fast"
        assert reason == "short simple query"
        assert confidence == 0.8
    ...
    def test_long_query_routes_to_quality(self):
        ...
        preset, reason, confidence = classify_query(long_query, _SAMPLE_SETTINGS)
    ...
    def test_code_block_routes_to_quality(self):
        query = "Fix this:\n```python\ndef foo(): pass\n```"
        preset, reason, confidence = classify_query(query, _SAMPLE_SETTINGS)
        assert preset == "quality"
    ...
    def test_custom_thresholds_respected(self):
        ...
        preset, reason, _ = classify_query("A medium query!", custom)
        assert preset == "balanced"
        assert reason == "moderate complexity"

    def test_confidence_is_between_0_and_1(self):
        for query in ["hi", "x" * 1000, "```code```"]:
            _, _, confidence = classify_query(query, _SAMPLE_SETTINGS)
            assert 0.0 <= confidence <= 1.0

    def test_boundary_at_simple_threshold(self):
        """Query exactly at simple_threshold (200 chars) with no signals → balanced."""
        query = "x" * 200
        preset, reason, _ = classify_query(query, _SAMPLE_SETTINGS)
        assert preset == "balanced"
        assert reason == "moderate complexity"
    ...
    def test_default_thresholds_when_routing_settings_default(self):
        """Default RoutingSettings still classifies short queries as fast."""
        defaults = Settings(models=_SAMPLE_SETTINGS.models)
...
class TestRouteQuery:
    ...
    def test_returns_routing_decision(self):
        decision = route_query("hello", _SAMPLE_SETTINGS)
        assert isinstance(decision, RoutingDecision)

    def test_fast_preset_resolves_to_fast_model(self):
        decision = route_query("hi", _SAMPLE_SETTINGS)
    ...
    def test_balanced_preset_resolves_to_balanced_model(self):
        query = "Can you explain how the agent framework works in this project and how agents are discovered? " * 3
        decision = route_query(query, _SAMPLE_SETTINGS)
